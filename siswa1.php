...
if ($search_query) {
    // Menggunakan LIKE pada NISN, Nama, Kelas, dan Jurusan
    $stmt = $koneksi->prepare("SELECT * FROM siswa WHERE nisn LIKE ? OR nama_siswa LIKE ? OR kelas LIKE ? OR jurusan LIKE ?");
    ...
    $stmt = $koneksi->prepare("SELECT * FROM siswa"); // Menampilkan semua data siswa jika tidak ada pencarian
}
...
<body>
    ...
    <div class="sidebar">
        ...
        <nav>
            ...
            <a href="siswa1.php" class="active">Data Siswa</a>
            <a href="cetak1siswa.php">Cetak</a>
        </nav>
    </div>
                ...
                <?php while ($row = $result->fetch_assoc()): ?>
                    <div class="card">
                        ...
                        <div class="card-actions">
                            <a href="edit1siswa.php?id=<?= $row['nisn'] ?>" class="btn-edit">Edit</a>
                            <a href="hapus1siswa.php?id=<?= $row['nisn'] ?>" class="btn-delete">Hapus</a>
                            <a href="view1siswa.php?id=<?= $row['nisn'] ?>" class="btn-view">View</a>
                        </div>
                    </div>
                <?php endwhile; ?>
            <?php else: ?>
